...
import ckanext.ab_process_state.helpers as helpers
from ckan.controllers.package import PackageController
from ckan.controllers.group import GroupController
from ckanext.ab_process_state.controllers.package import _save_new
from ckanext.ab_process_state.logic import action
...
class ProcessStatePlugin(plugins.SingletonPlugin):
    """
    ...
    def after_update(self, context, pkg_dict):
        ...
        return super(ProcessStatePlugin, self).after_update(context, pkg_dict)

                
    def after_create(self, context, pkg_dict):
        self._update_extra(context, pkg_dict)
        self._update_state(context, pkg_dict)
        return super(ProcessStatePlugin, self).after_create(context, pkg_dict)
    ...
    def before_map(self, map):
        controller = 'ckanext.ab_process_state.controllers:MemberAuthorizedController'
        ...
        map.connect('deactivate-multiple', '/datasets/deactivate-multiple',
                  controller='ckanext.ab_process_state.controllers:PackagesDeactivateController',
                  action='deactivate')
        map.connect('reactivate-multiple', '/datasets/reactivate-multiple',
                  controller='ckanext.ab_process_state.controllers:PackagesReactivateController',
                  action='reactivate')
        map.connect('delete-multiple' ,'/datasets/delete-multiple',
                  controller='ckanext.ab_process_state.controllers:PackagesDeleteController',
        ...
        map.connect('dashboard-filter-action', '/dashboard/datasets/filter-action',
                  controller='ckanext.ab_process_state.controllers:DashboardFilterActionPackagesController',
                  action='dashboard_datasets')
        map.connect('organization-filter-action', '/organization/{id}/filter-action',
                  controller='ckanext.ab_process_state.controllers:OrganizationFilterActionPackagesController',
                  action='read')
        map.connect('opendata-filter-action', '/opendata/filter-action',
                  controller='ckanext.ab_process_state.controllers:FilterActionPackagesController',
                  action='search')
    ...
    def update_config(self, config_):
        ...
        toolkit.add_resource('fanstatic', 'ab_process_state')
